              ...
              <Link
                ...
                className={`relative group text-[11px] tracking-[0.25em] uppercase transition-colors duration-300 ${
                  location.pathname === link.path
                    ...
                    : "text-stone-900/70 hover:text-stone-900"
                }`}
              >
                {link.name}
                <span
                  className={`absolute -bottom-1 left-0 h-[1.5px] bg-stone-900 transition-all duration-300 ease-out ${
                    location.pathname === link.path
                      ? "w-full"
          ...
          <div className="flex flex-col space-y-6 text-center items-center py-20">
            {allNavLinks.slice(0, 2).map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="text-xl font-serif text-stone-800 uppercase tracking-widest"
              >
                {link.name}
              </Link>
            ))}
            ...
            <div className="flex flex-col space-y-4 items-center">
              <span className="text-[10px] tracking-[0.3em] uppercase text-stone-400 font-bold">
                Treatments
              </span>
              {treatmentLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  className="text-lg font-serif text-stone-600 uppercase tracking-wider"
                >
                  {link.name}
                </Link>
              ))}
...
export const Footer: React.FC = () => {
  ...
  return (
    <footer
      id="footer"
      className="bg-[#eeeae7] text-stone-900 pt-[7.5rem] flex flex-col"
    >
        ...
        <div className="space-y-12">
          <div className="space-y-6">
            ...
            <div className="text-stone-600 font-light leading-relaxed max-w-md text-lg space-y-4">
              ...
              <p>
                We would love to help you meet your confidence goals through our
                aesthetic treatments. Please feel free to book your free
                in-clinic consultation to explore which services would be
                suitable for you.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="text-stone-600 font-light text-lg">
                Apex Clinic - based at AG PERMANENT COSMETICS ACADEMY
                <br />6 Carolgate, Retford DN22 6BU
              </p>
            </div>
            <div>
              <h3 className="text-sm font-bold uppercase tracking-widest text-stone-900 mb-2">
            ...
            <form className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-wider text-stone-400 font-bold">
                    First Name
                  </label>
                  <input
                    type="text"
                    className="w-full bg-transparent border-b border-stone-300 focus:border-stone-900 outline-none py-2 transition-colors font-light text-stone-900"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-wider text-stone-400 font-bold">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  rows={4}
                  className="w-full bg-transparent border-b border-stone-300 focus:border-stone-900 outline-none py-2 transition-colors font-light resize-none text-stone-900"
                ></textarea>
              </div>
              <button
                type="button"
      ...
      <div className="max-w-7xl mx-auto py-10 px-6 w-full border-t border-stone-200 flex flex-col md:flex-row justify-between items-center text-xs tracking-wider text-stone-500 gap-4">
        <p>
          © {new Date().getFullYear()} Apex Aesthetics Clinic. All rights
          reserved.
        </p>
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const baseStyles =
    "inline-block px-12 py-4 text-xs font-bold uppercase tracking-[0.2em] transition-all duration-300 ease-out rounded-none border text-center transform hover:scale-[1.02] active:scale-[0.98]";
  const variants = {
    primary:
      "bg-stone-900 text-white border-stone-900 hover:bg-white hover:text-stone-900 hover:shadow-lg hover:border-stone-900",
    outline:
      "bg-transparent text-stone-900 border-stone-900 hover:bg-stone-900 hover:text-white hover:shadow-md",
  };
